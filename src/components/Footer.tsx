...
import LegalModal, { LegalDocKey } from "./LegalModal";
...
const docLinks: { label: string; key: LegalDocKey }[] = [
  { label: "Политика конфиденциальности", key: "privacy" },
  { label: "Пользовательское соглашение", key: "terms" },
  { label: "Согласие на обработку персональных данных", key: "consent" },
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div>
              ...
              <ul className="space-y-2.5">
                {docLinks.map((l) => (
                  <li key={l.key}>
                    <button
                      onClick={() => setActiveDoc(l.key)}
                      className="text-sm text-slate-400 hover:text-white transition-colors leading-tight text-left"
                    >
                      {l.label}
                    </button>
                  </li>
          ...
          <div className="py-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-xs text-slate-600">
              © {year} ИП Кузовлева Алена Александровна
            </p>
            <p className="text-xs text-slate-700 text-center sm:text-right leading-relaxed">
              ИНН 771683589962 · ОГРН 1027700067328
            </p>
          </div>
        </div>
      </footer>

      <LegalModal docKey={activeDoc} onClose={() => setActiveDoc(null)} />
    </>
  );
}
